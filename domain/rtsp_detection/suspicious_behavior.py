    ...
    while True:
        ...
        if len(results[0].boxes) > 0 and any([box.id for box in results[0].boxes]):
                ...
                if cls == 0 and track_id is not None:  # Class 0 indicates a person
                    ...
                    if total_time > suspicion_threshold_time:
                        ...
                        detections_to_send.append(
                            {
                                "class": "Suspeito por permanência",
                            }
                        )
                    else:
            ...
            if len(suspects_ids) > 0 and any(
                ...
                frame_path = save_annotated_image(frame, str(image_folder_path))
                ...
                if detections_to_send:
                    if ws_client.is_connected():
                        ws_client.send_detection(
                            ...
                            frame_bytes=cv.imencode(".png", frame)[1].tobytes(),
                        )
                    else:
...
def detect_proximity_to_vehicle(
    ...
    if cap is None:
        logger.error(
            f"[SuspiciousBehavior_RTSPDetection] Could not open RTSP stream at {rtsp_url}."
            f" Detection cannot be performed."
        )
        return
    ...
    try:
        ...
        logger.error(f"[SuspiciousBehavior_RTSPDetection] {e} Detection cannot be performed.")
        return
    ...
    while True:
        ...
        if not success or frame is None:
            logger.error(
                "[SuspiciousBehavior_RTSPDetection] Could not read frame from " "RTSP stream."
            )
            break
