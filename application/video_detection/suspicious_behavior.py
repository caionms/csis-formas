"""
Módulo que executa detecção de comportamento suspeito por permanência em um vídeo.
"""
...
def update_tracking(
    tracker_results: list[Any], current_frame: int, tracking_data: TrackingData, track_buffer: int
) -> None:
    ...
    for track_id, data in list(tracking_data.items()):
        if data["last_seen"] != current_frame:
            ...
            if data["absent_frames"] > track_buffer:
                print(
                    f"Person with ID {track_id} was forgotten after {data['absent_frames']} "
                    f"frames of absence."
                )
            ...
            if data["total_present_frames"] > track_buffer:
                logger.info(
                    f"[SuspiciousBehaviorDetection][update_tracking] Alert! "
                    f"Person with ID {track_id} has been in the video for "
                    f"{data['total_present_frames']} frames."
                )


def main(
    video_path: str,
    supicious_frame_limit: int = 60,
    ...
    if not model_path.is_file():
        ...
        if not dropbox_manager.download(
            ...
            logger.error(
                "[SuspiciousBehaviorDetection] No model available and unable to "
                "download the model from Dropbox. Detection cannot be performed."
            )
            return
    ...
    while cap.isOpened():
        ...
        if len(results[0].boxes) > 0:
            ...
            update_tracking(
                tracker_results=[
                    track_id for i, track_id in enumerate(track_ids) if classes[i] == 0
                ],
            ...
            for box, track_id, cls, confidence in zip(boxes, track_ids, classes, confidences):
                ...
                plot_bbox(img=frame, class_id=cls, box_coordinates=box, label=label, color=color)
...
if __name__ == "__main__":
    ...
    # Para finalizar o programa, apertar Q depois de clicar na visualização do plot
    # ou fechar a janela que está sendo detectada
    # path = Path(__file__).parents[2] / "data" / "public_safety" / "1.mp4"
    # main("D:\\Documents\\TCC\\TIC\\2024-06\\EFGYarRUC2.mp4",
    # supicious_frame_limit=5, save_video=False, show_video=True)
